...
<quiz name="">
  <question>
    <p>
      在 <a href="./1.Exercise1.html">练习1</a>, 哪个图书馆分支的借阅量最高？
    </p>
  ...
  <question>
    ...
    <answer correct>您必须同时定义读写模块格式</answer>
    <answer>您必须定义读模块格式，但是写模块格式是可选的</answer>
    <answer>读模块格式是可选的，但是您必须定义写模块格式</answer>
    <answer>读模块和写模块格式都是可选的</answer>
  ...
  <question multiple>
    ...
    <answer correct>从大型数据库中读取</answer>
    <answer correct>从大型网络数据集中读取</answer>
    <answer>运行生产工作空间</answer>
    <answer>使用Creator和单个Emailer运行一个简单的工作空间以发送电子邮件</answer>
    <answercorrect>使用部分运行来逐步开发具有复杂工作流的工作空间</answer>
    <explanation>
      <p>
  ...
  <question multiple>
    ...
    <answer>生成工作空间将失败</answer>
    <answer correct>您可能看不到要查找的文件，因为您正在过滤以寻找其他文件格式</answer>
    <answer>技巧问题！设置格式之前，Workbench不允许您浏览源数据。</answer>
    <answer correct>Workbench可能会猜测您数据的格式错误，例如当它实际上是GeoJSON文件时就猜测它是JSON文件。</answer>
    <explanation>
  ...
  <question>
    ...
    <answer>输入格式名称</answer>
    <answer>从下拉列表中选择格式</answer>
    <answer>在读模块和写模块库中浏览格式</answer>
    <answer>通过选择具有已知文件扩展名的数据集</answer>
    <answer correct>以上都不是（这都是设置格式的所有有效方法）</answer>
    <explanation>
      <p>
