...
| 总体目标 | 为住宅垃圾收集创建边界 |
| 演示 | 读模块和写模块 |
| 启动工作空间 | C:\FMEData2019\Workspaces\DesktopBasic\Design-Ex3-Begin.fmw |
| 结束工作空间 | C:\FMEData2019\Workspaces\DesktopBasic\Design-Ex3-Complete.fmw |

在这里，我们继续一个重新定义垃圾收集时间表的项目。

根据分区类型，在前两个练习中，我们使用各种转换器将地址划分为五个不同的组。然后我们将数据写入Geopackage。
...
或者，您可以打开工作空间C：\ FMEData2019 \ Workspaces \ DesktopBasic \ Design-Ex3-Begin.fmw：
...
| :--- | :--- |
| 在输出'区域'上生成列表\(Generate List on Output 'Area'\) | Yes |
| '区域'列表名称\('Area'List Name\) | PointList |
...
通过在新的PointOnAreaOverlayer上运行工作空间来确认这是否正常工作。注意转换如何从两个缓存中提取数据的。AreaOnAreaOverlayer和SpatialSorter转换器：

![](./Images/Img3.238.Ex3.POAORun.png)

检查PointOnAreaOverlayer:Area输出端口要素。 选择一个要素将显示（在“要素信息”窗口中）列表属性及其所有值。
...
<td style="border: 1px solid darkorange">
<span style="font-family:serif; font-style:italic; font-size:larger">
虽然我们不再使用它, 但是要注意，"Assume Input Is Clean" 参数已从FME 2019.1中删除。
</span>
</td>
</tr>
</table>
...
删除Sampler转换器, 确保已连接PostalAddress和PointOnAreaOverlayer:Point输入端口。
