...
## The Top 30 ##
该[转换器的列表](https://www.safe.com/transformers/)从用户的反馈来计算，在Safe Software网站上按照被使用最多的顺寻来排序。掌握这些信息可以告诉我们在哪些方面指导我们的开发工作进行改进，但它也让用户了解他们在工作中最需要的（500多个）FME转换压器中的哪一个。

下表（最后更新于2018年6月）提供了最常用的30个变压器列表。Tester变压器每年都在列表中排名第一，突出了它的重要性。
...
除了用于转换几何的明显变换器（Clipper，Bufferer，Dissolver）和用于转换属性值的明显变换器（StringReplacer，Counter）之外，还有一些其他不同的变换器组。
