...
设置“从数据集导入”将允许我们导入Excel电子表格以用作指导。单击“确定”以添加写模块。

  
**4）导入要素类型**  
此时，系统会提示您选择要从中导入模式定义的数据集。应使用与写模块相同的值设置这两个字段。设置数据集参数如下：

| 读模块数据集 | C:\FMEData2019\Resources\DesktopBasic\AddressSchema.xlsx |
| :--- | :--- |


此文件是我们的指导/模板。单击“确定”接受值。将创建新要素类型以匹配所选的Excel模式。

**5）添加AttributeManager转换器**  
...
<br>**10) 删除属性**
<br>现在有了“Owners”字段，我们可以删除属性OWNERNM1和OWNERNM2。但是...操作顺序现在很重要。删除OWNERNM1和OWNERNM2**之前**，必须在参数对话框中创建Owners。
...
<br>现在是时候运行工作空间，以确保到目前为止我们所做的一切都能产生正确的结果。确保要素缓存已打开，然后通过单击AttributeManager转换器并选择“运行至此(Run to This)”来运行工作空间：
...
请注意，现在有一个名为\_list{}的list属性的条目。但是，这只是“*一般的*”列表属性- 它没有显示列表中的每个元素（值）。
...
<br>**14) 构建属性**
<br>最后一步是重新创建Street属性，而不用地址编号号作为前缀。一种方法是像使用所有者(owner)名称一样，使用文本编辑器来连接_list {1}，_ list {2}和_list {3}。但是，对于列表，我们可以使用特殊的ListConcatenator转换器。它只需要一个初步的步骤。
...
单击“确定”接受这些更改并关闭对话框。运行工作空间到这一点并检查输出。您会发现我们将街道名称的所有部分重新连接在一起。
...
<td style="border: 1px solid darkorange">
<span style="font-family:serif; font-style:italic; font-size:larger">
Excel写模块有一个参数叫做 <strong>Overwrite Existing File</strong>, 默认设置为<strong>No</strong>.
