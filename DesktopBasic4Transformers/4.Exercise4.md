...
请注意，在“表视图”中仅存在数据，如果打开“图形视图”，它将显示没有几何读写模块。我们将需要使用块(Block)将犯罪数据与我们的其他数据在空间上相关联。
...
运行工作空间（使用StringReplacer上的*Run to This*）并检查缓存以确保转换器按预期工作。“Number”列中的每个功能都应在末尾加上XX。  
