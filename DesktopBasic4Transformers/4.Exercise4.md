...
要解决此问题，只需在此处添加一个名为NumberCrimes的新属性。为其指定类型“number”和单元格宽度为6。来自Aggregator的数据现在将在写模块上具有其属性：

![](./Images/Img4.237.Ex4.AddCrimesAttr.png)

我们可以做的另一件事是检查Owners字段，看看它是否支持二进制值。遗憾的是，Excel没有二进制字段类型，所以我们不太可能能够读回那个字段，不管它是不是有密码。但是至少所有者的名字是隐藏的。
...
<td style="border: 1px solid darkorange">
<span style="font-family:serif; font-style:italic; font-size:larger">
该工作流非常复杂，数据来自多个来源，并且属性进行了大量更改。如果您有时间，最好在打包工作空间之前添加一些书签和注释以整理工作空间，以便将来需要返回该工作空间。
</span>
</td>
</tr>
</table>
